          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Customer Order Customer Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_customer_order_customer_idTouser?.id}`}>
                      {data?.user_customer_order_customer_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
              {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    User Customer Order Waitstaff Id Touser:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/users/view/${data?.user_customer_order_waitstaff_idTouser?.id}`}>
                      {data?.user_customer_order_waitstaff_idTouser?.email}
                    </Link>
                  </Text>
                </Flex>
              )}
